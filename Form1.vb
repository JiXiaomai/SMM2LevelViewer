    ...
    Public Sub InitPng()
        ...
        For i = 0 To H
            ...
            If (H - i) Mod 10 = 0 Then
                G.DrawString((H - i).ToString, Me.Font, Brushes.Black, 0, CSng(i * Zm))
            End If
        Next
        For i = 0 To W
            ...
            End If
            If i Mod 10 = 9 Then
                G.DrawString((i + 1).ToString, Me.Font, Brushes.Black, CSng(i * Zm), 0)
            End If
        Next

        If MapHdr.Theme = 2 Or MapHdr.Theme = 9 Then
            G.DrawLine(New Pen(Color.Red, 2), 0, CSng((H - MapHdr.LiqSHeight - 0.5) * Zm), CSng(W * Zm), CSng((H - MapHdr.LiqSHeight - 0.5) * Zm))
            G.DrawLine(New Pen(Color.Red, 2), 0, CSng((H - MapHdr.LiqEHeight - 0.5) * Zm), CSng(W * Zm), CSng((H - MapHdr.LiqEHeight - 0.5) * Zm))
        End If
    End Sub
    Public Sub InitPng2()
        Dim i As Integer
...
Err:
                    ...
                    If MapObj(i).LID + 1 = 0 And Not L Or MapObj(i).LID + 1 > 0 And L Or MapObj(i).ID = 9 Then
                        Select Case MapObj(i).ID
                            ...
                            Case 16
                                ...
                                For j = 0 To MapObj(i).W - 1
                                    If j = 0 Then
                                        G.DrawImage(GetTile(7, 3, 1, 1),
                                            CSng((j + MapObj(i).X \ 160) * Zm), H * Zm - CSng((MapObj(i).H + MapObj(i).Y \ 160) * Zm), Zm, Zm)
                                    ElseIf j = MapObj(i).W - 1 Then
                                        G.DrawImage(GetTile(9, 3, 1, 1),
                                            CSng((j + MapObj(i).X \ 160) * Zm), H * Zm - CSng((MapObj(i).H + MapObj(i).Y \ 160) * Zm), Zm, Zm)
                                    Else
                                        G.DrawImage(GetTile(8, 3, 1, 1),
                                            CSng((j + MapObj(i).X \ 160) * Zm), H * Zm - CSng((MapObj(i).H + MapObj(i).Y \ 160) * Zm), Zm, Zm)
                                    End If
                                Next
                            Case 71
                        ...
                        '	End If
                        '	G.DrawImage(Image.FromFile(P & "\img\CMN\F0.PNG"), LX, LY, Zm, Zm)
                        '	'G.DrawString(ObjEng(MapObj(i).CID), Button7.Font, Brushes.White, LX - 1, LY)
                        '	'G.DrawString(ObjEng(MapObj(i).CID), Button7.Font, Brushes.White, LX + 1, LY)
                        '	'G.DrawString(ObjEng(MapObj(i).CID), Button7.Font, Brushes.White, LX, LY - 1)
                        '	'G.DrawString(ObjEng(MapObj(i).CID), Button7.Font, Brushes.White, LX, LY + 1)
                        '	'G.DrawString(ObjEng(MapObj(i).CID), Button7.Font, Brushes.Black, LX, LY)
                        'End If
                        ...
                        If PR = "PW" Then PR = "B"
                        If PR.Length > 0 Then
                            'G.DrawString(PR, Button3.Font, New SolidBrush(Color.White), LX - 1, LY)
                            'G.DrawString(PR, Button3.Font, New SolidBrush(Color.White), LX, LY - 1)
                            'G.DrawString(PR, Button3.Font, New SolidBrush(Color.White), LX + 1, LY)
                            'G.DrawString(PR, Button3.Font, New SolidBrush(Color.White), LX, LY + 1)
                            'G.DrawString(PR, Button3.Font, New SolidBrush(Color.Black), LX, LY)
    ...
    Private Sub RefPic()
        ...
        Tile = Image.FromFile(PT & "\img\TILE\" & LH.GameStyle & "-" & MapHdr.Theme.ToString & ".png")
        TileW = Tile.Width \ 16
        InitPng()
        DrawObject(True)
    ...
    Public Sub DeMap(P1 As String, P2 As String)
        ...
        'decryptor.exe  从https://github.com/simontime/SMM2CourseDecryptor下载
        info = New System.Diagnostics.ProcessStartInfo With {
            .FileName = PT & "\MAP\decryptor.exe",
            .Arguments = P1 &
                    " " & PT & "\MAP\" & P2
        }
    ...
    Private Sub TrackBar1_Scroll(sender As Object, e As EventArgs) Handles TrackBar1.Scroll
        Label1.Text = "缩放：" & 2 ^ TrackBar1.Value
    End Sub

    Private Sub RefList(L As ListBox, IO As Boolean)
        Dim i As Integer, P As Integer, PR As String
        For i = 0 To MapHdr.ObjCount - 1
            If MapObj(i).CID = -1 Then
            ...
            Select Case MapObj(i).ID
                ...
                Case 14, 16, 71 '平台
                    For W = 1 To MapObj(i).W
                        ObjLocData(NowIO, Int(W - 0.5 + MapObj(i).X / 160), Int(MapObj(i).H + MapObj(i).Y / 160)) += GetItemName(MapObj(i).ID, LH.GameStyle).ToString & " "
                    Next
                Case 17 '桥
                    For W = 1 To MapObj(i).W
                        ObjLocData(NowIO, Int(W - 0.5 + MapObj(i).X / 160), Int(MapObj(i).H - 1 + MapObj(i).Y / 160)) += GetItemName(MapObj(i).ID, LH.GameStyle).ToString & " "
                    Next
                Case 87, 88 '斜坡
                Case 53, 94 '传送带
                Case 105 '夹子
                Case 55, 97 '门 传送箱
                    ...
                    For W = 1 To MapObj(i).W
                        For H = 1 To MapObj(i).H
                            ObjLocData(NowIO, Int(W - 0.5 + MapObj(i).X / 160), Int(H + MapObj(i).Y / 160)) += GetItemName(MapObj(i).ID, LH.GameStyle).ToString & Strings.Mid("ABCDEFGHJ", P + 1, 1) & " "
                        Next
                    Next
                Case Else
                    If MapObj(i).CID = -1 Then
                        For W = 1 To MapObj(i).W
                            For H = 1 To MapObj(i).H
                                ObjLocData(NowIO, Int(W - MapObj(i).W / 2 + MapObj(i).X / 160), Int(H + MapObj(i).Y / 160)) += GetItemName(MapObj(i).ID, LH.GameStyle).ToString & " "
                            Next
                        Next
                    Else
                        For W = 1 To MapObj(i).W
                            For H = 1 To MapObj(i).H
                                PR = ""
                                PR += IIf((MapObj(i).CFlag \ &H4000) Mod 2 = 1, "大", "")
                                PR += IIf((MapObj(i).CFlag \ &H8000) Mod 2 = 1, "伞", "")
                                PR += IIf((MapObj(i).CFlag \ 2) Mod 2 = 1, "飞", "")

                                ObjLocData(NowIO, Int(W - MapObj(i).W / 2 + MapObj(i).X / 160), Int(H + MapObj(i).Y / 160)) += GetItemName(MapObj(i).ID, LH.GameStyle).ToString & "(" & GetItemName(MapObj(i).CID, LH.GameStyle) & IIf(PR = "", "", "[" & PR & "]") & ")" & " "
                            Next
                        Next
                    End If
            End Select
    ...
    Private Sub Form1_DragDrop(sender As Object, e As DragEventArgs) Handles Me.DragDrop
        ...
        For Each path As String In files
            TextBox1.Text = path
        Next
        If Strings.Right(TextBox1.Text, 4).ToUpper = ".BCD" Then
            DeMap(TextBox1.Text, "TEMP.LVL")
            Threading.Thread.Sleep(3000)
            TextBox1.Text = PT & "\MAP\TEMP.LVL"
            isMapIO = True
            RefPic()
        Else
            RefPic()
